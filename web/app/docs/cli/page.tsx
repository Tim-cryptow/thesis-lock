...
export default function CliGuide() {
  return (
    <div>
      ...
      <Lead>
        The <Code>thesislock-cli</Code> package verifies anchors from the terminal or a CI pipeline,
        with no browser or wallet required. It hashes files locally and compares only the SHA-256
        digest against on-chain data; files are never uploaded.
      </Lead>

      <P>
        Every command also accepts <Code>--json</Code> for machine-readable output and{" "}
        <Code>--quiet</Code> for a single essential value, which makes the CLI easy to script.
      </P>

      <H2>Installation</H2>
      <P>
        The CLI depends on the sibling <Code>sdk/</Code> package, so build that first when working
        from the monorepo:
      </P>
...
node dist/bin/thesislock.js --help`}</CodeBlock>
      <P>
        Link the built package to put the <Code>thesislock</Code> command on your PATH. Once
        published to the registry, a global install also works:
      </P>
...
npm install -g thesislock-cli`}</CodeBlock>
      ...
      <H2>verify</H2>
      <P>
        Check whether a hash is anchored in any contract (single, batch, registry, proof NFT, or
        group). Exits <Code>0</Code> when anchored and <Code>1</Code> when not, so it works directly
        as a CI gate.
      </P>
  ...
  Timestamp: 2026-05-27T13:13:00.000Z`}</CodeBlock>
      <P>
        Batch owners are discovered automatically from registry events, and an explicit owner can be
        supplied with <Code>--owner</Code>:
      </P>
      <CodeBlock language="bash">{`thesislock verify <hash> --owner SP000...`}</CodeBlock>
      <P>
        Use <Code>--quiet</Code> for a bare <Code>true</Code> or <Code>false</Code>, or{" "}
        <Code>--json</Code> for a structured result.
      </P>
...
thesislock verify <hash> --json`}</CodeBlock>
      ...
      <H2>hash</H2>
      <P>
        Compute the SHA-256 digest of one or more files. For each file the CLI prints the filename,
        size, and 64-character hex hash.
      </P>
      <CodeBlock language="bash">{`thesislock hash thesis.pdf
thesislock hash chapter1.pdf chapter2.pdf chapter3.pdf`}</CodeBlock>
      <P>
        Add <Code>--verify</Code> to check each digest against the chain in the same step. The exit
        code is <Code>1</Code> if any file is missing an anchor.
      </P>
...
thesislock status SPMXTB2P571VMJP2ZG812P2H964S1XVTCDC8QNYX`}</CodeBlock>
      ...
      <H2>search</H2>
      <P>
        Search anchors across all contracts. The query type is auto-detected: 64-hex searches by
        hash, a Stacks address by wallet, and anything else as a label substring. Results print as a
        table; add <Code>--json</Code> for machine-readable output, and <Code>--limit</Code> to cap
        the rows.
      </P>
...
thesislock search SPMXTB2P571VMJP2ZG812P2H964S1XVTCDC8QNYX --limit 5`}</CodeBlock>
      ...
      <H2>batch</H2>
      <P>
        Hash every file in a directory. For each file the CLI prints the path relative to the
        scanned directory, size, and hash, then a summary line. Add <Code>--recursive</Code> to
        descend into subdirectories, <Code>--exclude</Code> for comma-separated glob patterns, and{" "}
...
thesislock batch ./papers --verify --json`}</CodeBlock>
      ...
      <H2>Scripting</H2>
      <P>
        <Code>--quiet</Code> emits a single value per command, ideal for shell substitution, and{" "}
        <Code>--json</Code> pairs well with <Code>jq</Code>.
      </P>
...
thesislock batch ./papers --verify --json | jq -r '.[] | select(.anchored == false) | .path'`}</CodeBlock>
      ...
      <P>
        ...
        <Code>completions/</Code> directory of the CLI package and complete every command and flag.
      </P>
...
#   autoload -U compinit && compinit`}</CodeBlock>
      ...
      <P>
        Use <Code>verify</Code> or <Code>hash --verify</Code> as a step that fails when a document
        is not anchored:
      </P>
