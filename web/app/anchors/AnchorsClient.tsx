...
import { TAGS_CHANGED_EVENT, getTagColor, getTagsForHash } from "@/lib/tags";
...
export default function AnchorsPage() {
  ...
  const loadHistory = useCallback(
    async (owner: string) => {
      ...
      setError(null);
      try {
        const [c, recent] = await Promise.all([getAnchorCount(owner), getRecentAnchors(owner)]);
        setCount(c);
  ...
  const presentTemplateIds = Array.from(
    new Set(
      parsedEntries.map((p) => p.parsed.templateId).filter((id): id is string => Boolean(id)),
    ),
  );
  ...
  return (
    <div className="flex-1 max-w-3xl mx-auto px-6 py-12 w-full">
      ...
      <p className="text-foreground/70 mb-8">{t("anchors.intro")}</p>

      {!address ? (
        <div className="rounded-lg border border-foreground/10 bg-card p-10 text-center">
          <p className="text-foreground/70 mb-6">{t("anchors.connectPrompt")}</p>
          <button
            onClick={connectWallet}
            disabled={connecting}
        ...
        <>
          <div className="flex items-center gap-2 mb-4">
            <span className="text-xs text-foreground/50 uppercase tracking-wide mr-1">View</span>
            <button
              type="button"
          ...
          {view === "calendar" ? (
            <div className="rounded-lg border border-foreground/10 bg-card p-4 sm:p-6">
              <ContributionGraph
                ...
                onSelectDay={(d) => setCalendarSelected((c) => (c === d.date ? null : d.date))}
              />
              <DayDetail
                day={
                  calendarSelected
            ...
            <div className="space-y-3" role="list">
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap pb-1">
                <p className="text-xs text-foreground/50">{t("anchors.compareHint")}</p>
                <div className="flex gap-2">
                  {filteredEntries.length > 0 && (
                    <Link
                      href="/report"
                      onClick={() =>
                        stageReportInput(filteredEntries.map(({ entry }) => ({ hash: entry.hash })))
                      }
                  ...
                  >
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                      <input
                        ...
                        disabled={!selected.includes(entry.hash) && selected.length >= 2}
