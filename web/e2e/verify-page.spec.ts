...
async function settleLookup(page: Page, expectation: () => Promise<void>): Promise<void> {
...
test.describe("verify page", () => {
  test("rejects an invalid hash format", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Invalid hash format." })).toBeVisible();
    await expect(page.getByText("A valid hash is 64 lowercase hex characters.")).toBeVisible();
  });
  ...
  test("reports an unanchored hash", async ({ page }) => {
    ...
    await settleLookup(page, async () => {
      await expect(page.getByText("This hash has not been anchored.")).toBeVisible({
        timeout: 5_000,
      });
    });
  });

  test("shows verification details for a known anchored hash", async ({ page }) => {
    test.slow();
    await page.goto(`/v/${KNOWN_ANCHORED_HASH}`);

    await expect(page.getByRole("heading", { name: "Anchor record" })).toBeVisible();
    ...
    await expect(page.getByText("This hash has not been anchored.")).toHaveCount(0);
  });

  test("offers badge embedding and share links for a verified anchor", async ({ page }) => {
    test.slow();
    await page.goto(`/v/${KNOWN_ANCHORED_HASH}`);

    // The badge and share cards only render once the anchor resolves.
    await settleLookup(page, async () => {
      await expect(page.getByRole("heading", { name: "Embed a badge" })).toBeVisible({
        timeout: 5_000,
      });
    });
