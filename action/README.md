# ThesisLock Verify Action

A reusable GitHub Action that verifies a document hash is anchored on the
Stacks blockchain via [ThesisLock](https://thesis-lock.vercel.app). Use it in CI
to confirm a dataset, paper, or release artifact is still backed by an on-chain
proof-of-existence timestamp.

The action is self-contained: it reads the public Hiro mainnet API directly and
needs no wallet, secret, or signing key. Verification is read-only.
...
Or verify a hash you already know:
...
Provide either `hash` or `file`. When `file` is set it takes precedence and the
action computes the file's SHA-256 digest locally.
...
1. If `file` is supplied, the action reads it and computes a SHA-256 digest. The
   file never leaves the runner.
2. It queries the public Hiro mainnet API with read-only contract calls:
...
3. The first contract that holds the hash determines `source`, `block`, and
   `label`. If none do, the hash is not verified.
4. When `fail-on-unverified` is `true` (the default), an unverified hash fails
   the step so a broken proof can gate a release.
